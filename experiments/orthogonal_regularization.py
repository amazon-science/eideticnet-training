...
def get_parser():
    ...
    parser.add_argument(
        ...
        choices=[
            func_name
            for func_name in dir(eideticnet.losses)
            if "loss" in func_name
        ],
        required=False,
        help="Orthogonality regularlizer from eideitcnet.losses",
    )
    parser.add_argument(
        "--lambda-ortho",
        type=float,
...
if __name__ == "__main__":
    ...
    if args.orthogonality_loss:
        args.orthogonality_loss_func = getattr(
            eideticnet.losses, args.orthogonality_loss
        )
        for module_name, module in model.named_modules():
            if isinstance(module, torch.nn.Conv2d):
                hook = partial(
