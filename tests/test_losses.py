...
class TestGini:
    ...
    def test_backward(self):
        ...
        y = gini_loss(x, use_abs=False)
        y.backward()
        optimizer.step()
        # With a high learning rate the Gini coefficient will be low after one
        # update step.
        assert torch.allclose(gini_loss(x, use_abs=False), torch.zeros(1))
        # And none of the population will have zero wealth.
        assert (x == 0.0).sum() == 0

    def test_smoke_conv2d(self):
        """Test that 4-d weight tensors do not raise an exception."""
